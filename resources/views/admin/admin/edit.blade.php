...
<article class="page-container">
	<form class="form form-horizontal" id="form-admin-add" action="/ciwei/admin/{{$res->id}}" method="post">
	@if (count($errors) > 0)
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-5 col-sm-5">
			<input type="text" class="input-text" value="{{$res->phone}}" placeholder="" id="phone" name="phone">
		</div>
	</div>
	<div class="row cl">
	...
	<div class="row cl skin-minimal">
		...
		<div class="formControls col-xs-5 col-sm-5">
			@foreach($role as $row)
			<div class="check-box">
		    	<input type="checkbox" value="{{$row->id}}" name="role[]" {{in_array($row->id,$ar)?'checked':''}}>
		    	<label for="checkbox-2">{{$row->rolename}}</label>
		 	</div>
		 	@endforeach
		</div>
	</div>
