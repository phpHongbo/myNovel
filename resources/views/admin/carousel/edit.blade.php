...
<div class="page-container">
	...
	@endif
	<form class="form form-horizontal" id="form-article-add" method="post" action="/ciwei/lunbo/{{$res->id}}" enctype="multipart/form-data">
	   {{csrf_field()}}
	   {{ method_field('PUT') }}
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-5 col-sm-5">
				...
				<select name="status" class="select">
					...
					<option value="0" @if($res->type == 0) selected @endif>开启</option>
					<option value="1" @if($res->type == 1) selected @endif>禁用</option>
				</select>
				</span>
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-5 col-sm-5">
				<input type="text" class="input-text" value="{{$res->urlid}}" placeholder="" id="ids" name="urlid">
			</div>
		</div>
		<div class="row cl">
			...
			<img class="avatar size-XXXL radius" src="{{$res->image}}">
			</div>
		</div>
		<div class="row cl">
